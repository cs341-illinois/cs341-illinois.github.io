...
The CEO hands you a small USB drive and a slip of badly coffee-stained paper with "sha256: ebddfa36f5af..." - but the rest of the hexadecimal digits are unreadable, plus some unreadable scratch notes taken from Thaddea's desk about memory allocators, the words <tt>memset</tt> <tt>jq</tt> plus various doodles of different Drones.
...
1. Was Thaddea's <tt>jq</tt> drone warnings correct? Can you show us how to reproduce the OOM bug when heap memory is limited to 96 MB for mimalloc heap alloctors? For example, Thaddea's notes showed the following would fail with OOM `ulimit -v 98304; LD_PRELOAD=$PWD/mimalloc/build/libmimalloc.so.3 jq "length" batch-99-2m.json`; can you build mimalloc and reproduce that failure?
2. You should know a bit about `glibc` malloc, `jemalloc` and `mimalloc` e.g, what features is each one known for? Which allocator would you recommend for the drone? Prove to us that you verified that your choice can parse the given geo json data within the 96 MB memory ulimit.
3. The team will ask you to explain the `ulimit -v` and `LD_PRELOAD` portions i.e. what does each part of line do? What is `jq` anyway?
4. How does the performance of your own part2 allocator compare to `jemalloc`, `mimalloc` on this data? Do you have any good reasons to recommend or not recommend your own allocator? What is `tcmalloc` good for and why are you not even evaluating it?
5. If we cleaned up the json file e.g., removed the unnecessary `null` entries, are there good reasons (e.g. processing time, memory usage) to recommend jemalloc, glibc, or, mimalloc over the other allocators?
...
Later,  - a flash of inspiration as you put the pieces together - you suddenly understand the fate of Thaddea, you burst into see the CEO's office barely able to speak while you catch your breath...

 * What will you tell the CEO? Where is Thaddea, what happened to her, and what actions do you immediately recommend they do at the demo site? Just for fun - ungraded -  Enter you best guess / brilliant deduction / action recommendation within this [google form](https://forms.gle/zUQNcio3yiYHm9Hk6).
